...
const AdminSidebar = () => {
  ...
  const [reservationsNeedingConfirmationCount, setReservationsNeedingConfirmationCount] = useState(0);
  ...
  // Fetch reservations needing confirmation count
  useEffect(() => {
    const fetchCount = async () => {
      try {
        const response = await adminAPI.getReservationsNeedingConfirmationCount();
        if (response.data.success) {
          setReservationsNeedingConfirmationCount(response.data.count);
        }
      } catch (error) {
        console.error('Erreur lors du chargement du nombre de réservations nécessitant une confirmation:', error);
      }
    };
            ...
            {menuItems.map((item, index) => (
              <li key={index} className="mb-1">
                ...
                >
                  ...
                  {expanded && (
                    <span className="sidebar-text ms-3 d-flex align-items-center justify-content-between flex-grow-1">
                      <span>{item.label}</span>
                      {item.path === '/admin/reservations' && reservationsNeedingConfirmationCount > 0 && (
                        <span className="badge bg-danger ms-2">
                          {reservationsNeedingConfirmationCount}
                        </span>
                      )}
                    </span>
                  )}
                  {!expanded && item.path === '/admin/reservations' && reservationsNeedingConfirmationCount > 0 && (
